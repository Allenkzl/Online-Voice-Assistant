...
**采样率复核（决定性实验）**：让模型"一字不差地重复我刚才那句话"，输入录音 2.56s（11 音节）：

| 假设采样率 | 输出时长 | 与输入比 | 判定 |
|---|---|---|---|
| **44100** | **2.20 / 2.28 s** | **0.86~0.89x** | ✅ 语速一致，正确 |
| 32000 | 3.03 / 3.14 s | 1.18~1.22x | ✗ 偏慢 |
| 24000 | 4.04 / 4.19 s | 1.57x | ✗ 明显拖慢 |
| 16000 | 6.06 / 6.28 s | 2.36x | ✗ 完全不对 |
...
**处理**：`audio.normalize_loudness()` 在转码前做响度归一化（目标 RMS 默认 0.09，峰值上限 0.97，
增益上限 4x，不削波），日志新增 `E2E_LEVEL rms_in/gain/rms_out`，`Reply.meta` 增加 `gain`；
可通过 `glm_voice_target_rms` / `WAKE_GLM_VOICE_TARGET_RMS` 调整。
音色本身的（相对千问 TTS 更"合成"）差异无法靠后处理消除，需要人耳最终判定是否接受。
